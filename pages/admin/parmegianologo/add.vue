...
const form = reactive({
  ...
  particular: 2,
  obs_carne:
    "Ponto estava perfeito, suculenta e bem temperada. A crosta estava crocante",
  obs_molho: "Bastante molho e bem temperado. ",
  obs_acompanhamentos:
    "Acompanhamentos bem feitos e saborosos. Pratos bem apresentados",
  obs_restaurante:
    "Ambiente agradável, atendimento rápido e cordial. Preço justo, serve bem 2 pessoas",
...
const onFormSubmit = async (e) => {
  ...
  if (e.valid) {
    const { data, error } = await client.from("parmegianas").insert(
      {
        restaurante_id: form.selectedRestaurante?.id,
        prato: form.prato,
        preco: form.preco,
        carne: form.carne,
        molho: form.molho,
    ...
    >
      ...
      <div class="flex flex-col gap-8">
        ...
        <FloatLabel variant="over">
          ...
          <Textarea
            name="obs_acompanhamentos"
            rows="5"
            cols="30"
            style="resize: none"
            v-model="form.obs_acompanhamentos"
          />
        </FloatLabel>
        <FloatLabel variant="over">
          <InputNumber
          ...
          <Textarea
            name="obs_restaurante"
            rows="5"
            cols="30"
            style="resize: none"
            v-model="form.obs_restaurante"
          />
        </FloatLabel>
      </div>
      <div class="max-w-2xl mx-auto bg-white p-6 rounded-lg shadow-md">
        ...
        <!-- Status -->
        <p v-if="statusMessage" class="mt-4 text-sm text-gray-600">
          {{ statusMessage }}
        </p>
      </div>
